...
def predict_one(pipeline, item_id, text, question="", knowledge=""):
    ...
    print(f"  Model    : {fr['model_arch'].upper()}  |  Threshold: {fr['threshold']}")
...
def main():
    ...
    if not Path(args.model_path).exists():
        print(f"\nERROR: No trained model found at {args.model_path}")
        print("Please run 'python train.py --arch bert' first.")
        sys.exit(1)

    print("\nLoading transformer pipeline...")
    pipeline = DeepLearningHallucinationPipeline.load(
        model_path = args.model_path,
        model_name = args.model_name,
        threshold  = args.threshold,
    )
    print(f"Model loaded: {pipeline.model_arch.upper()}  |  Threshold: {pipeline.threshold}")

    print("\n" + "="*65)
    print("  DEEP LEARNING HALLUCINATION DETECTOR — Predict Mode")
    print("="*65)
    print("\n  Choose mode:")
