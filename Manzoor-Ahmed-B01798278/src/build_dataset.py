...
if __name__ == "__main__":
    ...
    truthfulqa_rows = load_truthfulqa_csv("../data/raw/truthfulqa/TruthfulQA.csv")
    all_rows.extend(truthfulqa_rows)
    print(f"TruthfulQA: {len(truthfulqa_rows)} rows")

    if INCLUDE_FEVER:
        fever_rows = load_fever_jsonl("../data/raw/fever/train.jsonl")
        all_rows.extend(fever_rows)
        print(f"FEVER: +{len(fever_rows)} rows  (total: {len(all_rows)})")
    else:
        print("FEVER: skipped")

    halueval_rows = load_halueval_jsonl("../data/raw/halueval/qa_data.json")
    all_rows.extend(halueval_rows)
    print(f"HaluEval: +{len(halueval_rows)} rows  (total: {len(all_rows)})")
